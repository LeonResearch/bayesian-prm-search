...
def save_dataset(dataset, config):
    if config.push_to_hub:
        ...
        config.output_dir += f"/{inference_model}/{prm_model}"
        Path(config.output_dir).mkdir(parents=True, exist_ok=True)
        file_name = f"{config.output_dir}/{config.approach}_n-{config.n}_completions-{num_questions}.jsonl"
        dataset.to_json(file_name, lines=True)
        logger.info(f"Saved completions to {file_name}")
